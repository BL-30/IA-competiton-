...
# %%
def dijkstra(graph,start_vertex,targets):
    min_heap = []
    heapq.heappush(min_heap,(0,start_vertex))
    routing_table = {}
    explored_vertex = []
    parent_table = {}
    parent_table[start_vertex] = None

    while len(min_heap) > 0:
      distance,current_vertex = heapq.heappop(min_heap)
      explored_vertex.append(current_vertex)
      parent = parent_table[current_vertex]
      routing_table[current_vertex] = (parent,distance)
      if all_in(targets, explored_vertex):
        return routing_table
    
      for enfant in graph[current_vertex]:
            
            if enfant not in explored_vertex:
               distance_through_v = distance + graph[current_vertex][enfant]
               heapq.heappush(min_heap, (distance_through_v,enfant))

               if (distance_through_v,enfant) in min_heap:
                 parent_table[enfant] = current_vertex

    return routing_table

# %%
def give_score(maze_map,current_vertex,neighbors):
  score = []
  routing_table = dijkstra(maze_map,current_vertex,neighbors)
  
  for vertex in neighbors :
    if vertex != current_vertex:
       score.append((routing_table[vertex][1],vertex,routing_table))
       
  return score

# %%
def greedy (graph, current_vertex, vertices_to_visit):
  global best_route

  score = give_score(graph,current_vertex,vertices_to_visit)

  (dist,next_piece_location,routing_table) = min(score)

  best_route = find_route(routing_table,current_vertex,next_piece_location)

  ##print(best_route)

  return next_piece_location
...
def find_route(routing_table, source_location, target_location): 
  ...
  while current_location != source_location:
    route.append(routing_table[current_location][0]) ##on rajoute le parent de current_location au chemin
    current_location = routing_table[current_location][0]
  route.reverse()
...
# %%
def rat_movement(graph,player_location,vertices_to_visit):
  global best_route
  global next_piece 
  global player_loc

  if next_piece not in vertices_to_visit:
     next_piece = greedy(graph,player_location,vertices_to_visit)
  ...
  target = best_route[0]
  del best_route[0]
  move =  move_from_locations(player_location,target)
  player_loc = target
  return move
...
def preprocessing (maze_map, maze_width, maze_height, player_location, opponent_location, pieces_of_cheese, time_allowed) :
   ...
   next_piece = greedy(maze_map,player_loc,pieces_of_cheese)
...
def turn (maze_map, maze_width, maze_height, player_location, opponent_location, player_score, opponent_score, pieces_of_cheese, time_allowed) :
    global player_loc
    return rat_movement(maze_map,player_loc,pieces_of_cheese)